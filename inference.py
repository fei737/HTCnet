...
def resolve_img_path(base_dir, idx, prefixes=("",), exts=(".png", ".jpg", ".jpeg")):
    idx = str(idx)
    candidates = []
    if idx.isdigit():
        i = int(idx)
        variants = [idx, f"{i:04d}"]
    else:
        variants = [idx]
    for p in prefixes:
        for v in variants:
            for e in exts:
                candidates.append(os.path.join(base_dir, f"{p}{v}{e}"))
    for c in candidates:
        if os.path.exists(c):
            return c
    raise FileNotFoundError(f"未找到图像: tried={candidates[:6]} ...")
...
def predict_and_show(data_dir, checkpoint_path, idx="0700", n_classes=14):
    ...
    rgb_path = resolve_img_path(os.path.join(data_dir, "NYURGB"), idx, prefixes=("",))
    hha_path = resolve_img_path(os.path.join(data_dir, "HHA_datasets"), idx, prefixes=("hha_", ""))
    rgb_raw = cv2.cvtColor(cv2.imread(rgb_path), cv2.COLOR_BGR2RGB)
    hha_raw = cv2.cvtColor(cv2.imread(hha_path), cv2.COLOR_BGR2RGB)
