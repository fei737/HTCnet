...
class EdgeAwareDWConv2d(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        x = x.permute(0, 3, 1, 2).contiguous()
        local_detail = x - F.avg_pool2d(x, kernel_size=3, stride=1, padding=1)
        x = self.pointwise(F.gelu(self.dwconv(local_detail))) * self.gate(x)
        x = x.permute(0, 2, 3, 1).contiguous()
        return x
...
def _stable_masked_softmax(scores, mask, clamp_val=1e4):
    """Numerically stable masked softmax for the geometry-decay attention.

    Replaces the previous ``nan_to_num``-everything approach, which masked the
    real source of NaNs/Infs (unbounded geometry-decay logits overflowing under
    bf16/fp16 autocast) instead of preventing it. Here we:

      1. add the decay mask in fp32,
      2. bound the combined logits to a finite range (the decay penalty can be
         arbitrarily negative for distant patches with large depth gaps),
      3. subtract the row-wise max before ``exp`` for stability,
      4. fall back to a uniform row only if an entire row is non-finite, so a
         single bad entry can never propagate a NaN through the matmul.

    Args:
        scores: (..., L) attention logits, expected in fp32.
        mask:   (..., L) additive decay mask, broadcastable to ``scores``.
    Returns:
        (..., L) softmax weights in fp32.
    """
    scores = scores + mask
    scores = torch.clamp(scores, min=-clamp_val, max=clamp_val)
    # Guard against an all-non-finite row (degenerate depth prior) producing
    # a NaN max; such rows collapse to a uniform distribution instead.
    finite = torch.isfinite(scores)
    scores = torch.where(finite, scores, scores.new_full((), -clamp_val))
    row_max = scores.amax(dim=-1, keepdim=True)
    weights = torch.softmax(scores - row_max, dim=-1)
    return weights
...
class Decomposed_GSA(nn.Module):
    ...
    def forward(self, x: torch.Tensor, rel_pos):
        ...
        sin = torch.nan_to_num(sin.to(dtype=x.dtype), nan=0.0, posinf=1e4, neginf=-1e4)
        cos = torch.nan_to_num(cos.to(dtype=x.dtype), nan=0.0, posinf=1e4, neginf=-1e4)
        mask_h = torch.nan_to_num(mask_h.float(), nan=-1e4, posinf=1e4, neginf=-1e4)
        mask_w = torch.nan_to_num(mask_w.float(), nan=-1e4, posinf=1e4, neginf=-1e4)
        ...
        qk_mat_w = qr_w.float() @ kr_w.float().transpose(-1, -2)
        qk_mat_w = _stable_masked_softmax(qk_mat_w, mask_w.transpose(1, 2))
        v = torch.matmul(qk_mat_w.to(dtype=v.dtype), v)

        qr_h = qr.permute(0, 3, 1, 2, 4)
        kr_h = kr.permute(0, 3, 1, 2, 4)
        v = v.permute(0, 3, 2, 1, 4)

        qk_mat_h = qr_h.float() @ kr_h.float().transpose(-1, -2)
        qk_mat_h = _stable_masked_softmax(qk_mat_h, mask_h.transpose(1, 2))
        output = torch.matmul(qk_mat_h.to(dtype=v.dtype), v)
