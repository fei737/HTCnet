...
class OHEMCrossEntropyLoss(nn.Module):
    def __init__(self, thresh=0.7, ignore_index=255, min_kept=30000, n_classes=41, class_weights=None):
        ...
        self.criterion = nn.CrossEntropyLoss(
            weight=self.class_weights, ignore_index=ignore_index, reduction="none"
        )
        self.mining_criterion = nn.CrossEntropyLoss(
            weight=None, ignore_index=ignore_index, reduction="none"
        )
...
class DGRQCombinedLoss(nn.Module):
    def __init__(
        self,
        lambda_lovasz=0.5,
        lambda_dice=0.4,
