...
class PFNet(nn.Module):
    def __init__(self, n_classes=14, pretrained_path=None, return_aux=False):
        ...
        if not positive_indices:
            raise RuntimeError(f"Encoder channels are invalid: {encoder_channels}")
        self.edge_fused_idx = positive_indices[0] - 1
        edge_channels = encoder_channels[positive_indices[0]]
    ...
    def forward(self, rgb, hha):
        ...
        edge_logits = self.edge_head(fused_list[self.edge_fused_idx])
