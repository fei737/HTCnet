...
def register_plot_hooks(model):
    hook_targets = {
        "rgb_backbone": model.encoder.rgb_backbone,
        "structure_encoder": model.encoder.structure_encoder,
        "edge_descriptor": model.encoder.edge_descriptor,
        "guide_pyramid": model.encoder.guide_pyramid,
        "prompt_encoder": model.encoder.prompt_encoder,
        "fusion_shallow": model.encoder.fusion_stages[0],
        "fusion_deep": model.encoder.fusion_stages[-1],
        "context_head": model.encoder.context_head,
        "decoder": model.decoder,
    }
    if model.encoder.reliability_head is not None:
        hook_targets["reliability_head"] = model.encoder.reliability_head
    return [
...
def main():
    ...
    if args.architecture_variant == "factorized" and isinstance(checkpoint_config, dict):
        ...
        if checkpoint_encoding != args.geometry_encoding:
            raise ValueError(
    ...
    rgb_t, hha_t = normalize_modalities(
        rgb_resized,
        hha_resized,
        device=device,
    ...
    if args.plot_features:
        rgb_shallow = pick_pyramid_feature(activations.get("rgb_backbone"), deep=False)
        rgb_deep = pick_pyramid_feature(activations.get("rgb_backbone"), deep=True)
        geometry_reliability = first_tensor(activations.get("reliability_head"))
        geometry_structure = first_tensor(activations.get("structure_encoder"))
        prompt_shallow = nested_tensor(activations.get("prompt_encoder"), 0)
        prompt_deep = nested_tensor(activations.get("prompt_encoder"), -1)
        geometry_edges = first_tensor(activations.get("edge_descriptor"))
        guide_shallow = nested_tensor(activations.get("guide_pyramid"), 0)
        fused_shallow = first_tensor(activations.get("fusion_shallow"))
        fused_deep = first_tensor(activations.get("fusion_deep"))
        context_feature = first_tensor(activations.get("context_head"))
        decoder_logits = nested_tensor(activations.get("decoder"), 0)
        decoder_edge_logits = nested_tensor(activations.get("decoder"), 1)
        ...
        show_overlay_panel(axes[1, 1], rgb_resized, tensor_to_map(geometry_edges, display_size), "HHA Edge Descriptor", cmap="magma")
        show_overlay_panel(axes[1, 2], rgb_resized, tensor_to_map(guide_shallow, display_size, reduction="mean"), "Geometry Guide Pyramid")
        show_overlay_panel(axes[1, 3], rgb_resized, tensor_to_map(rgb_shallow, display_size, reduction="mean"), "RGB Shallow Feature")

        show_overlay_panel(axes[2, 0], rgb_resized, tensor_to_map(geometry_reliability, display_size, reduction="mean"), "Geometry Reliability")
        show_overlay_panel(axes[2, 1], rgb_resized, tensor_to_map(fused_shallow, display_size, reduction="mean"), "Shallow Geometry Fusion")
        show_overlay_panel(axes[2, 2], rgb_resized, tensor_to_map(rgb_deep, display_size, reduction="mean"), "RGB Deep Feature")
        show_overlay_panel(axes[2, 3], rgb_resized, tensor_to_map(prompt_deep if prompt_deep is not None else geometry_structure, display_size, reduction="mean"), "Deep Geometry Prompt")

        show_overlay_panel(
            axes[3, 0],
            rgb_resized,
            tensor_to_map(context_feature if context_feature is not None else fused_deep, display_size, reduction="mean"),
            "Pyramid Context Feature",
        )
        show_overlay_panel(
            axes[3, 1],
            rgb_resized,
        ...
        show_overlay_panel(
            axes[3, 3],
            rgb_resized,
            tensor_to_map(prompt_shallow, display_size, reduction="mean"),
            "Shallow Geometry Prompt",
            cmap="magma",
        )
        ...
        show_overlay_panel(
            axes[4, 0],
            rgb_resized,
            tensor_to_map(geometry_structure, display_size, reduction="mean"),
        ...
        show_overlay_panel(
            axes[4, 1],
            rgb_resized,
            tensor_to_map(decoder_edge_logits if decoder_edge_logits is not None else inspect_edge_logits, display_size, apply_sigmoid=True),
            "Decoder Edge Logits",
            cmap="magma",
            alpha=0.65,
        )
        show_overlay_panel(
            axes[4, 2],
            rgb_resized,
            tensor_to_map(fused_deep, display_size, reduction="mean"),
            "Deep Geometry-Guided Fusion",
            cmap="magma",
        )
